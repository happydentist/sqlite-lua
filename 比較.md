...
```mermaid
graph TR
    subgraph hoelzro 架構 (臨場型)
        A1[SQL Row 1] --> B1[🆕 建立全新 Lua VM] --> C1[執行運算] --> D1[❌ 銷毀 Lua VM]
        A2[SQL Row 2] --> B2[🆕 建立全新 Lua VM] --> C2[執行運算] --> D2[❌ 銷毀 Lua VM]
    end

    subgraph abiliojr 架構 (長駐大數據型)
        E[🔌 連線初始化] --> F[🟢 建立永久 LuaJIT VM]
        F --> G[💾 載入 logic.lua 並註冊自訂 SQL 函數]
        
        H1[SQL Row 1] -->|直接調用記憶體常駐函數| G
        H2[SQL Row 2] -->|直接調用記憶體常駐函數| G
        H3[SQL Row 3] -->|直接調用記憶體常駐函數| G
        G --> I[🚀 JIT 動態優化為機器碼，逼近原生 C 速度]
    end
```
